...
describe('Validation', function() {
    it('should be defined', function() {
        ...
        expect(typeof validation.validateProperty).toEqual('function');
    });

    it('should require a schema', function () {
        expect(function() {
            validation.validate();
        }).toThrow('IllegalArgumentsException: Schema missing!');
    ...
    describe('common', function() {
        ...
        describe('required', function() {
            ...
            it('should pass if value is truthy (or 0)', function () {
                subject.name = 'John';
                expect(_.isEmpty(validation.validate(subject, schema))).toEqual(true);
                subject.name = 1;
                expect(_.isEmpty(validation.validate(subject, schema))).toEqual(true);
                subject.name = [];
                expect(_.isEmpty(validation.validate(subject, schema))).toEqual(true);
            ...
            it('should fail if value is falsy', function () {
                subject.name = null;
                expect(validation.validate(subject, schema).name).toEqual(true);
                subject.name = void 0;
                expect(validation.validate(subject, schema).name).toEqual(true);
                subject.name = '';
                expect(validation.validate(subject, schema).name).toEqual(true);
        ...
        describe('translated error messages', function () {
            it('should return translated error message when rule has msgKey property', function () {
                ...
                expect(validation.validate(participant, schema).name).toEqual(dictionary.t(msgKey + '.required'));
                participant.name = 2;
                expect(validation.validate(participant, schema).name).toEqual(dictionary.t(msgKey + '.type'));
            });
        });
    });
    ...
    describe('number', function() {
        ...
        describe('type:number', function() {
            ...
            it('should not accept a value of different type', function() {
                subject.age = 'John';
                expect(validation.validate(subject, schema).age).toEqual(true);
            });
        });
        ...
        describe('type:decimal', function () {
            ...
            it('should not accept a value of different type', function () {
                ...
                expect(validation.validate(subject, schema).average).toEqual(true);
            });
        });
    ...
    describe('array', function() {
        describe('type', function() {
            ...
            it('should not accept a value of different type', function() {
                ...
                expect(validation.validate(subject, schema).languages).toEqual(true);
                subject.languages = {};
                expect(validation.validate(subject, schema).languages).toEqual(true);
            });
        });
    ...
    describe('type:object', function() {
        ...
        it('should not accept a value of different type', function() {
            subject.competences = [];
            expect(validation.validate(subject, schema).competences).toEqual(true);
    ...
    describe('Multiple fields', function() {
        ...
        it('should return true if any of the rules are not met', function() {
            ...
            expect(result.name).toEqual(true);
            expect(result.age).toEqual(true);
            expect(result.languages).not.toBeDefined();
        });
    });
    ...
    describe('Validating a single property', function () {
        ...
        it('should return true when msgKey is not defined and the result is errous', function () {
            expect(validation.validateProperty('name', 'gg', schema)).toEqual(true);
        });

        it('should return undefined when validation is successfull', function () {
            expect(validation.validateProperty('name', 'ggall', schema)).toEqual(void 0);
        });

        it('should return a translated error message if msgKey is defined', function () {
            schema.name.msgKey = 'error.participant.name';
            expect(validation.validateProperty('name', 'ggallin', schema)).toEqual(dictionary.t(schema.name.msgKey + '.maxLength'));
        });
    });
});